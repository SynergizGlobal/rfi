...
public interface RFIRepository extends JpaRepository<RFI, Long> {
	...
	@Query(value = """
			...
			LEFT JOIN rfi_validation rv on r.id = rv.rfi_id_fk
	...
	@Query(value = "SELECT COUNT(id) FROM rfi_data where is_deleted !=1 ", nativeQuery = true)
	int countOfAllRfiCreatedSoFar();

	int countByAssignedPersonClientAndIsDeletedFalse(String assignedTo);
	...
	@Query(value = "select r.rfi_id , r.id,rv.id, rv.action as status, rv.remarks as remarks, rv.validation_authority as valdationAuth, rv.comment from rfi_data as r\r\n"
	...
	@Query(value = "select r.rfi_id , r.id,rv.id, rv.action as status, rv.remarks as remarks, validation_authority as valdationAuth, rv.comment  from rfi_data as r\r\n"
			+ "right join rfi_validation as rv on r.id = rv.rfi_id_fk\r\n"
	...
	@Query(value = "    SELECT r.rfi_id,\r\n" + "           r.id,\r\n" + "           rv.id,\r\n"
			...
			+ "			   validation_authority as valdationAuth, rv.comment  \r\n" + "    FROM rfi_data r\r\n"
	...
	@Query(value = """
			SELECT
			  ...
			  ANY_VALUE(ico.post_test_report_path) As testResultContractor,
	            ...
	            ANY_VALUE(ua.attachmentData) AS attachmentData
			FROM rfi_data r
			  ...
			  LEFT JOIN rfi_enclosure re
	           ...
	           LEFT JOIN (
			    SELECT rfi_id_fk,
			           GROUP_CONCAT(
			             CONCAT(file_name, '::', REPLACE(IFNULL(description,''),'::',''), '##')
			             ORDER BY id SEPARATOR '||'
			           ) AS attachmentData
			    FROM rfi_attachments
			    GROUP BY rfi_id_fk
			) ua ON ua.rfi_id_fk = r.id
						WHERE r.id = :id
			GROUP BY r.id;
			""", nativeQuery = true)
	...
	@Query(value = """
			    SELECT
			        ...
			        ANY_VALUE(rv.action) AS validationStatus,
			        ANY_VALUE(r.project_name) AS project,
			        ANY_VALUE(r.work_short_name) AS work,
			        ANY_VALUE(r.contract_short_name) AS contract,
			    ...
			    LEFT JOIN rfi_inspection_details AS i
			        ON r.id = i.rfi_id_fk AND i.uploaded_by = 'Engg'
			    LEFT JOIN rfi_validation AS rv
			        ON rv.rfi_id_fk = r.id
			    WHERE
			        (:#{#obj.project} IS NULL OR :#{#obj.project} = '' OR r.project_name = :#{#obj.project})
			        AND (:#{#obj.work} IS NULL OR :#{#obj.work} = '' OR r.work_short_name = :#{#obj.work})
			        AND (:#{#obj.contract} IS NULL OR :#{#obj.contract} = '' OR r.contract_short_name = :#{#obj.contract})
			    GROUP BY r.id
			    ORDER BY r.created_at DESC
			""", nativeQuery = true)
	List<RfiLogDTO> listAllRfiLogItAdmin(@Param("obj") RfiLogFetchDTO obj);
	...
	@Query(value = """
			    ...
			    LEFT JOIN rfi_validation AS rv
			        ON rv.rfi_id_fk = r.id
			    WHERE
			        r.created_by = :#{#obj.userName}
			        AND (:#{#obj.project} IS NULL OR :#{#obj.project} = '' OR r.project_name = :#{#obj.project})
			        AND (:#{#obj.work} IS NULL OR :#{#obj.work} = '' OR r.work_short_name = :#{#obj.work})
			        AND (:#{#obj.contract} IS NULL OR :#{#obj.contract} = '' OR r.contract_short_name = :#{#obj.contract})
			    GROUP BY r.id
			    ORDER BY r.created_at DESC
			""", nativeQuery = true)
	List<RfiLogDTO> listAllRfiLogByCreatedBy(@Param("obj") RfiLogFetchDTO obj);

	// 🟩 Engineer (with filters + obj binding)
	@Query(value = """
			    SELECT
			        r.id AS id,
			        ANY_VALUE(r.rfi_id) AS rfiId,
			    ...
			    LEFT JOIN rfi_validation AS rv
			        ON rv.rfi_id_fk = r.id
			    WHERE
			        r.assigned_person_client = :#{#obj.userName}
			        AND (:#{#obj.project} IS NULL OR :#{#obj.project} = '' OR r.project_name = :#{#obj.project})
			        AND (:#{#obj.work} IS NULL OR :#{#obj.work} = '' OR r.work_short_name = :#{#obj.work})
			        AND (:#{#obj.contract} IS NULL OR :#{#obj.contract} = '' OR r.contract_short_name = :#{#obj.contract})
			    GROUP BY r.id
			    ORDER BY r.created_at DESC
			""", nativeQuery = true)
	List<RfiLogDTO> listAllRfiLogByAssignedBy(@Param("obj") RfiLogFetchDTO obj);
	...
	@Query(value = """
			    ...
			    ORDER BY r.created_at DESC
			""", nativeQuery = true)
	List<RfiLogDTO> listAllRfiLogByDyHod(@Param("obj") RfiLogFetchDTO obj);
	...
	@Query("SELECT COUNT(r) FROM RFI r WHERE r.status IN :statuses AND r.createdBy = :createdBy AND r.isDeleted = false ")
	...
	@Query("SELECT COUNT(DISTINCT r) FROM RFI r JOIN r.inspectionDetails i "
			...
			+ " AND r.isDeleted = false AND r.createdBy = :createdBy ")
	long countRejectedInspectionsByCreatedBy(@Param("createdBy") String createdBy);

	@Query(value = """
			SELECT COUNT(DISTINCT r.id)
			FROM rfi_data r
			JOIN rfi_inspection_details i 
			    ON r.id = i.rfi_id_fk 
			    AND i.uploaded_by = 'engg'
			LEFT JOIN rfi_validation rv 
			    ON r.id = rv.rfi_id_fk
			WHERE 
			    (i.test_insite_lab = 'rejected' 
			     OR rv.action = 'rejected')
			AND r.is_deleted = 0 
			AND r.status = 'INSPECTION_DONE'
			""", nativeQuery = true)
	...
	@Query(value = " SELECT COUNT(DISTINCT r.id)\r\n"
			...
			+ "    ON r.id = rv.rfi_id_fk\r\n"
			+ "WHERE \r\n"
			+ "    i.test_insite_lab = 'accepted'\r\n"
			+ "    AND (rv.action = 'approved' OR rv.action IS NULL)\r\n"
			+ "    AND r.is_deleted = 0\r\n"
			+ "    AND r.status = 'INSPECTION_DONE';\r\n"
			+ "", nativeQuery = true)
	long countAcceptedInspections();
	
	@Query("SELECT COUNT(DISTINCT r) FROM RFI r JOIN r.inspectionDetails i " + "WHERE r.status = 'INSPECTION_DONE' "
			+ " AND r.isDeleted = false ")
	long countClosedInspections();
	...
	@Query(value = """
		    ...
		    WHERE r.status = 'INSPECTION_DONE'
		      AND i.test_insite_lab = 'Accepted'
		      AND (rv.action = 'approved' OR rv.action IS NULL)
		      AND r.assigned_person_client = :assignedTo
		""", nativeQuery = true)
	...
	@Query(value = """
		    ...
		    LEFT JOIN rfi_validation rv
		        ON r.id = rv.rfi_id_fk
		    WHERE r.status = 'INSPECTION_DONE'
		      AND (i.test_insite_lab = 'rejected' OR rv.action = 'rejected')
		      AND r.assigned_person_client = :assignedTo
		""", nativeQuery = true)
	...
	@Query("SELECT COUNT(DISTINCT r) FROM RFI r JOIN r.inspectionDetails i " + "WHERE r.status = 'INSPECTION_DONE' "
			+ "AND r.assignedPersonClient = :assignedTo AND r.isDeleted = false" )
	...
	@Query(value = """
		    ...
		    WHERE r.status = 'INSPECTION_DONE'
		      ...
		      AND (
		         ...
		         OR r.name_of_representative = :userName
		         OR r.assigned_person_client = :userName
		      )
		""", nativeQuery = true)
	...
	@Query(value = """
				...
				ON r.id = rv.rfi_id_fk
				WHERE r.status = 'INSPECTION_DONE'
				  AND  (i.test_insite_lab = 'rejected' 
							     OR rv.action = 'rejected')
				  AND (
				        r.created_by = :userName
				     OR r.name_of_representative = :userName
	...
	@Query("SELECT COUNT(DISTINCT r) FROM RFI r JOIN r.inspectionDetails i " + "WHERE r.status = 'INSPECTION_DONE' "
			+ "AND (r.createdBy = :userName OR r.nameOfRepresentative = :userName OR r.assignedPersonClient = :userName) AND r.isDeleted = false ")
	...
	// 🟩 Regular User / Representative (with filters + obj binding)
	@Query(value = """
			    SELECT
			        r.id AS id,
			        ANY_VALUE(r.rfi_id) AS rfiId,
			    ...
			    LEFT JOIN rfi_validation AS rv
			        ON rv.rfi_id_fk = r.id
			    WHERE
			        LOWER(r.name_of_representative) = LOWER(:#{#obj.userName})
			        AND (:#{#obj.project} IS NULL OR :#{#obj.project} = '' OR r.project_name = :#{#obj.project})
			        AND (:#{#obj.work} IS NULL OR :#{#obj.work} = '' OR r.work_short_name = :#{#obj.work})
			        AND (:#{#obj.contract} IS NULL OR :#{#obj.contract} = '' OR r.contract_short_name = :#{#obj.contract})
			    GROUP BY r.id
			    ORDER BY r.created_at DESC
			""", nativeQuery = true)
	List<RfiLogDTO> listAllRfiLogByRepresentative(@Param("obj") RfiLogFetchDTO obj);
	...
	@Query(value = """
			    ...
			    ORDER BY r.created_at DESC
			""", nativeQuery = true)
	List<RfiLogDTO> listAllRfiLogByDataAdmin(@Param("obj") RfiLogFetchDTO obj);

	@Query(value = "select distinct project_name from rfi_data where project_name is not null and project_name <> ''", nativeQuery = true)
	List<String> listAllProjectFilterRfiLog();

	@Query(value = "select distinct work_short_name from rfi_data where work_short_name is not null and work_short_name <> ''", nativeQuery = true)
	List<String> listAllWorkFilterRfiLog();

	@Query(value = "select distinct contract_short_name from rfi_data where contract_short_name is not null and contract_short_name <> ''", nativeQuery = true)
	List<String> listAllContractFilterRfiLog();
